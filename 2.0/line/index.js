 ...
 * TODO 简化这个大文件
 * */
;KISSY.add("gallery/kcharts/2.0/bar/index",function(S,Raphael,BaseChart,Promise,Anim,Util,D,E){
   //==================== utils start ====================
   var each    = S.each
     , map     = S.map
     , indexOf = S.indexOf
   ...
   /**
    * 过滤series
    *  eg. [{name:"Group1",data:[ [xval,yval] , ... ]}]
    * @param xconfig {Object}
    *  eg. {range:{min:min,max:max,interval}}
    * @param yconfig {Object}
     ...
     for(var i=0,l1=series.length;i<l1;i++){
       ...
       if(!series[i].hided){// 只要没有被隐藏的数据
         ...
         for(var j=0,l2=serie.data.length;j<l2;j++){
           ...
           // result 为 false 直接跳过，说明略掉这条数据
           // result 为 [null,null] 则表示画线时断开
           if(result){
   ...
   * */
   ...
   var props = {
     ...
     render:function(){
       ...
       var w2 = this.get("width");
       var h2 = this.get("height");

       var padding = this.getPadding();
       var paddingx = padding.paddingLeft;
       var paddingy = padding.paddingTop;
       ...
       // 1.1 算出series数据的范围
       // {values:[1,3,2,...],dates:[unix_time_stamp]}
   ...
   var RealTime = BaseChart.extend(props);
   return RealTime;
 },{
   requires:[
